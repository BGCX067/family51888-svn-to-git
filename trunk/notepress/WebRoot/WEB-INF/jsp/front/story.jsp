...
<head>
<title>${story.title}-AJava.org</title>
...
<div class="container">
	...
	<div class="span-19 last">
		...
		<div class="content-color round-box">
			<p><a href="${pageContext.request.contextPath}/s-${story.id}"><img src="${pageContext.request.contextPath}${story.thumbnail}" title="${story.title}" width="160" height="90" class="left"/></a><strong><a href="${pageContext.request.contextPath}/s-${story.id}">${story.title}</a></strong>--${story.content}</p>
		</div>
		<!-- 故事摘要 -->
		
		<!-- 故事文件夹列表 -->
		<div class="clearfix">
		<c:forEach items="${folderList}" var="folder" begin="0" step="1"
			varStatus="index">
				<div class="span-3 title-color round-box"><h5><a href="${pageContext.request.contextPath}/f-${folder.id}" class="clearfix center">${folder.title}</a></h5></div>
		</c:forEach>
		</div>
		<!-- 故事文件夹列表 -->
		
		<!-- 最新文件 -->
		<div><h5 class="content-color round-box">${story.title}最新发布的文件</h5></div>
		<c:forEach items="${newFileList}" var="file" begin="0" step="1"
			varStatus="index">
			<div class="content-color round-box">
		...
		<!-- 最热文件 -->
		<h5 class="content-color round-box">${story.title}最受欢迎的文件</h5>
		<c:forEach items="${hotFileList}" var="file" begin="0" step="1"
			varStatus="index">
			<c:set var="ct" value="${file.createTime/86400000}"/>
			<c:set var="ut" value="${file.updateTime/86400000}"/>
			<div class="content-color round-box">
					<h2><a href="${pageContext.request.contextPath}/file-${file.id}" target="_blank">${file.title}</a><span class="fancy">--${file.viceTitle}</span></h2>
					<p>${file.excerpt}<a href="${pageContext.request.contextPath}/file-${file.id}" target="_blank">查看详细内容</a></p>
					<h5 class="fancy">作者：${file.author}<c:if test="${!empty file.translator}"> - 译者：${file.translator}</c:if> - 来源：${file.source}<c:if test="${!empty file.sourceUrl}"><a href="${file.sourceUrl}" target="_blank">查看原文</a></c:if></h5>
