...
<head>
<title>${story.title}_${folder.title}-AJava.org</title>
...
<div class="container">
	...
	<div class="span-19 last">
		...
		<c:forEach items="${folderList}" var="folder" begin="0" step="1"
			varStatus="index">
				<div class="span-3 title-color round-box <c:if test="${folder.id==param.folderId}">selected-border</c:if>"><h5 class="center"><a href="${pageContext.request.contextPath}/f-${folder.id}" class="clearfix">${folder.title}</a></h5></div>
		</c:forEach>
		</div>
		<!-- 故事文件夹列表 -->
		
		<!-- 最新文件 -->
		<c:forEach items="${fileList.result}" var="file" begin="0" step="1" varStatus="index">
			<div class="content-color round-box">
		...
		<c:set value="${pageContext.request.contextPath}/f-${param.folderId}" target="${fileList}" property="url"/> 
		<h5 class="content-color round-box">${fileList.pageHtml}</h5>
		<!-- 最新文件 -->
