...
private void Member_Check()
  {
     if(Request.Cookies["Member"]==null)
        {
          UserName="";
          Response.Write("<script type='text/javascript' src='/e/js/order.js'></script><script type='text/javascript'>NotLogin('"+SiteId+"');</script>");
          Response.End();
        }
      else
        {
...
private void Order_Add()
 {   
   ...
   sql="insert into pa_orders(site_id,username,order_id,name,tel,province,city,email,postcode,address,beizhu,sendway,send_spending,operator) values("+SiteId+",'"+UserName+"','"+Str_orderid+"','"+Name+"','"+Tel+"','"+Province+"','"+City+"','"+Email+"','"+PostCode+"','"+Address+"','"+Beizhu+"','"+SendWay+"',"+SendSpending+",'')";
   OleDbCommand  comm=new OleDbCommand(sql,conn);
  ...
  if(dr.Read())
   {
    ...
    if(MailTo!="")
     {
      SendMail Mail=new SendMail(int.Parse(SiteId));   
      bool sendresult=Mail.Send(MailTo,"",Email,Mail_subject,Mail_Body);//发邮件
     }
   } 
  dr.Close();
}
