...
<br>发货状态：<%=SendState=="1"?"<span class='send_success'>已发货</span>":"未发货"%>
<br><%if(PayState!="1"){%><asp:Button Text="付 款"  onclick="Pay_Order" onclientclick="return payck()"  runat="server" cssclass="bt"/> &nbsp; <%}%><input type="button" value="返 回"   class="bt" onclick="location.href='<%=Get_Url("mem_odidx")%>'">
  </td> 
   </tr>
   <tr>
