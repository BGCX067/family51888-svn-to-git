...
private void List_Bind()
   {
     if(UserName!="")
      {
     sql="select site_id,sort_id,id,title,lanmu_id,sublanmu_id,thedate,static_dir,static_file,zdy_url, permissions,clicks,html from "+TheTable+" where checked=1 and username='"+UserName+"' order by thedate desc";
