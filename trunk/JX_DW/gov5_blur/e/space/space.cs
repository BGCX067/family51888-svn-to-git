...
private void Get_Member()
 {
   ...
   sql="select id,username,m_type,space_title,space_introduction,introduction,truename,space_clicks,head_image,space_banner from pa_member where checked=1 and id="+UID;
   ...
   if(dr.Read())
    {
      ...
      MType_Id=int.Parse(dr["m_type"].ToString());
   ...
   if(MType_Id==0)
    ...
    {
     sql="select m_type from pa_member_type where id="+MType_Id;
     comm=new OleDbCommand(sql,conn);
     dr= comm.ExecuteReader();
     if(dr.Read())
      {
       Member_Type=dr["m_type"].ToString();
      }  
     dr.Close();
    }
 }
...
private void Table_Bind()
  {
   sql="select id,thetable,table_name from pa_table where [inspace]=1 order by xuhao";
...
private void Get_MemberSet()
 {
   string Space_Open="";
   sql="select  space_open from pa_member_set";
   OleDbCommand comm=new OleDbCommand(sql,conn);
   OleDbDataReader dr=comm.ExecuteReader();
   if(dr.Read())
    {
...
private void Get_LoginMember(string User,int MType_Id)
 {
   ...
   if(dr.Read())
    {
       Login_Member_Type=dr["m_type"].ToString();
    }  
   dr.Close();
