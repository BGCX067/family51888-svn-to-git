...
function Load_Comments(Table,DetailId,Page,GoTop) //读取评论
 {
  ...
  $("c_title_sign").innerHTML="发布评论";
  if($("c_name_sign")!=null)$("c_name_sign").innerHTML="用户：";
  if($("c_yzm_sign")!=null)$("c_yzm_sign").innerHTML="验证码：";
  if($("c_username_sign")!=null)
  {
   $("c_username_sign").innerHTML="用户名：";
   $("c_pass_sign").innerHTML="密码：";
  }
  if($("c_anonymous_sign")!=null)$("c_anonymous_sign").innerHTML="匿名";
  if($("c_username")!=null)$("c_username").value="游客";
  $("c_submit").value="提交";
  if(GoTop==null)
   {
     GoTop=true;
...
function cquote(id)
 {
   var obj1=$("c_username_"+id);
   var obj2=$("c_date_"+id);
   var obj3=$("c_content_"+id);
   var c_quote=$("c_quote"); 
   var c_quote_tips=$("c_quote_tips"); 
...
function check_comments() //评论输入检查
  {
   ...
   if(IsMember==0)
    ...
    Add_Comments($("c_siteid").value,$("c_table").value,$("c_detailid").value,c_quote.value,c_content.value,c_name.value,c_yzm.value,$("c_checked").value,0);
   }
  else
   {
      var UserName,Pass;
   ...
   Add_Comments($("c_siteid").value,$("c_table").value,$("c_detailid").value,c_quote.value,c_content.value,UserName,Pass,$("c_checked").value,1);
   }
 }