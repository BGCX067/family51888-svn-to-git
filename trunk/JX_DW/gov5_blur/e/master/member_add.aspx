...
<%if(Act=="add"){%><div id="tabdiv">
<ul>
<li id="tab" name="tab" onclick="location.href='member_list.aspx?tid=<%=Request.QueryString["tid"]%>&group=<%=Request.QueryString["group"]%>'">会员管理</li>
<li id="tab" name="tab" style="font-weight:bold">增加会员</li>
</ul>
</div><%}%>
...
for(int i=0;i<RegTypeCount;i++)
 {
%><option value="<%=Amids[i]%>" <%=CurrentTid==Amids[i]?"selected":""%>><%=Amtypes[i]%></option>
<%}%></select>
</td>
</tr>
<tr>
